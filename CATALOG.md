...
Start from `dist/catalog-entry.toml`, generated by `arc plugin build` from Cargo
metadata. Add the committed archive URL and SHA-256. The SDK build helper reports
the ABI/wire revisions; if an older SDK does not report them, the draft marks them
as missing instead of assuming compatibility with the Arc CLI. `id`, `name`,
